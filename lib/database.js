...
module.exports = {
    ...
    findLastMessagesWithUsers:function(messagesSkip, messagesCount, onSuccess, onError){
        db.manyOrNone('SELECT users.name as author, users.avatar, message.message_text, message.send_time, video.path as ' +
            'attach FROM users, message left join video on message.id = video.id_message WHERE users.id = ' +
    ...
    addMessageWithAttach: function(userId, message, onError){
        db.one('INSERT INTO message(send_time, message_text, id_user) VALUES(to_timestamp($1 / 1000.0), $2, $3) RETURNING id', [message.send_time,
            message.message_text, userId])
            .then(function(newMessageId){
                db.none('INSERT INTO video(path, id_message) VALUES($1, $2)', [message.attach, newMessageId.id])
                    .catch(onError);
            })
            .catch(onError);
    }
