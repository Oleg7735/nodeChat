...
var db = pgp('postgresql://postgres:753114@localhost/testChat');
module.exports = {
    init:function (connectionString) {
        //TODO: использовать эту функцию в app.js для задания строки подключения в зависимости от окружения
    ...
    findLastMessagesWithUsers:function(messagesSkip, messagesCount, onSuccess, onError){
        db.manyOrNone('SELECT users.name as author, message.message_text, message.send_time FROM users, message WHERE users.id = ' +
            'message.id_user ORDER BY message.send_time DESC LIMIT $1 OFFSET $2', [messagesCount, messagesSkip])
            .then(onSuccess)
            .catch(onError);
    },
    //Объект user должен содержать поля name, email, password
